...
namespace ei {
    ...
    linked_point_t arc (const Point &center, float radius, int start_angle, int end_angle) {
        ...
        for (int i = 0; i <= nbpts; i++) {
            Point p = Point(
                    round(radius * cosf(start_angle * M_PI / 180.f + (float) i * angle / (float) nbpts) + center.x()),
    ...
    linked_point_t rounded_frame (const Rect &rect, float radius, bt_part part) {
        ...
        if (part == BT_TOP) {
            list.push_back(
                    Point(rect.top_left.x() + 2 * rect.size.width() / 3, rect.top_left.y() + rect.size.height() / 2));
            list.push_back(
                    Point(rect.top_left.x() + rect.size.width() / 3, rect.top_left.y() + rect.size.height() / 2));
            return list;
        }
    ...
    void draw_line (surface_t surface, const Point &start,
        ...
        if (clipper)
            al_set_clipping_rectangle(clipper->top_left.x(), clipper->top_left.y(), clipper->size.width(),
                                      clipper->size.height());
        al_draw_line(start.x(), start.y(), end.x(), end.y(),
                     al_map_rgba(color.red, color.green, color.blue, color.alpha), 1);
    }
            ...
            if (start.y() != end.y()) {
                ...
                if (edge_table[current_scanline] == NULL) {
                    ...
                    while (tmp_edge->next != NULL && (tmp_edge->next->y_max <= current_edge->y_max)) {
                        if ((tmp_edge->next->y_max == current_edge->y_max) &&
                            (tmp_edge->next->x_min > current_edge->x_min))
                            break;
                ...
                for (pos.x() = ceil(current_edge->x_min); pos.x() <= floor(current_edge->next->x_min); pos.x()++) {
                    if (clipper == NULL ||
                        ((clipper->top_left.x() <= pos.x()) && (pos.x() < clipper->top_left.x() + clipper->size.width())
                         && (clipper->top_left.y() <= pos.y()) &&
                         (pos.y() < clipper->top_left.y() + clipper->size.height()))) {
